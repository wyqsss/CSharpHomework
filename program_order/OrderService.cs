    ...
    class OrderService
    {
        ...
        public List<Order> QueryByGoodsName(string goodsName)
        {
            ...
            foreach (Order order in orderDict.Values)
            {
                ...
                //    }
                //}
                var C = from item in order.Details where item.Goods.Name == goodsName select item;
        ...
        public string Getbyprice(Order order)
        {
            var g_1 = from item in order.Details where item.Goods.Price >= 1000 select item;
            return g_1.ToString();
        }
    }
}
